...
How `change_<SPECIES>_final_final.gff` is produced: each species' gene annotation, with its
gene IDs swapped for the *D. melanogaster* gene symbol of the same orthogroup. This is the
**gene-renaming step (stage 0b)** that the pipeline docs list as uncommitted — the code has
since turned up in `to_organize/` (untracked), and the step has been re-run and checked
against the lab's originals (see [Verification](#verification)).

The finished files for 26 species are in `to_organize/final_final/` (and `final_final.7z`).
...
Zenodo 18453526 ── annotations.tar.gz ──► gffs/<SPECIES>_final.gff.gz   (301 species)
     ...
     Step_2_Remove_Duplicate_gene_names_10_31.py                        │
                                    │                                   │
     hog_og/HOG_OG_association_gene_names_without_duplicates_10_31.tsv  │
                    │                          │                        │
     make_Dmel_output.py                       │                        │
                    │                          │                        │
            Dmel_output.tsv                    │                        │
...
All scripts named here live in `to_organize/`. Line references below point at those files.
...
### 2. The HOG table — `hog_og/HOG_OG_association_gene_names_without_duplicates_10_31.tsv`
...
`Dmel_HOG_association.tsv` is not in `to_organize/` either. Neither can be re-run as they
stand; the table they produced is present, and it is what `ReVamp_Final.py` reads.
...
`make_Dmel_output.py` rebuilds it from the HOG table (it just copies the `HOG` and
`DROSOPHILA_MELANOGASTER` columns for non-empty rows, lines 13–14). The copy in
`to_organize/` is a rebuild; the original was never kept. The rebuild reproduced the
arizonae output exactly, so the two are equivalent at least for that species.
...
**`ReVamp_Final.py` and everything in `to_organize/` are evidence of what the lab did. Do not
edit them, fix them, or write output next to them.** To re-run the step, copy the script
somewhere else, change only the paths in the copy, and write output outside the repository.
...
2. **Make a copy of the script outside the repository** and change only these paths in the
   ...
   | Line | Set to |
   |---|---|
   | 6 `DmelPath` | `to_organize/Dmel_output.tsv` |
   | 11 `hogogPath` | `to_organize/hog_og/HOG_OG_association_gene_names_without_duplicates_10_31.tsv` |
...
annotation, and compared with `final_final/change_DROSOPHILA_ARIZONAE_final_final.gff`:
...
stage — `pipeline-scripts-output/`, which pairs Cyp genes with nearby TEs — was run on
...
path-only copies of `repeatOpp.py`, `Locate_TE.py` and `CleanAnnasse.py` and the committed
RepeatMasker `.out`.

**Baseline.** The committed `DA_Files/DROSOPHILA_ANANASSAE_final_withDmelNames.gff` is
**truncated** — 4,330 lines, one chromosome, ending mid-line (`NC_057927.1⇥Gno`) — so it
cannot be what the committed results were made from; they span 6 chromosomes. The committed
`filtered.gff` (166 lines) is, however, `repeatOpp.py`'s output from the full file. Re-running
`Locate_TE.py` and `CleanAnnasse.py` from it reproduces the committed
...
annotation — ananassae is not among the 26 in `final_final/` — then run through the same three
scripts.

| (unique rows) | `_withDmelNames` (committed) | `final_final` |
|---|---|---|
...
the committed result, with the same Cyp name. Nothing is extra or conflicting.
...
Cyp genes in multi-copy orthogroups.
...
Ayush's `_withDmelNames` files, which is also what the committed ananassae example used.
...
The small `to_organize/D_Ananassae_GenesAffectedByTE.txt` (April, 98 rows, 23 unique, two
genes) is an early partial run; all its rows are in the committed table.
...
`process_gff_line`) instead of raw text replacement, which avoids the substring defect. Which
dataset the rest of the pipeline used is unresolved — see
[`04-gaps-and-provenance.md`](../pipeline/detailed/04-gaps-and-provenance.md), G2.
