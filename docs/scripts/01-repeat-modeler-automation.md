...
> **This document is about code that is not in this repository, and that the lab did not
> write.** `repeat-modeler-automation` was written in September 2026 to replace the lab's
> manual RepeatModeler and RepeatMasker sessions, and was split out into its own repository on
> 2026-09-15. It is kept here because it is the most complete statement of what the lab's
> stages 1 and 2 had to cope with — the day-long runs, the reboots, the parallelism — and
> because the pipeline documentation still refers to it.
>
> For what the lab actually did, see
> [`../pipeline/detailed/02-stage-reference.md`](../pipeline/detailed/02-stage-reference.md).
> Line references below are to that separate repository, not to anything under
> [`../../evidence/`](../../evidence/).
...
| `rmodeler.conf` | 65 | the live config (differs from the example — see below) |
...
Line references are against the files in the `repeat-modeler-automation` repository as of
its split from this one, and were re-verified at that point.
...
not a template — and `genomes_dhakad` ties it to the `Dhakad_et_al_2025` dataset named in the
archive ([`docs/ocr/04`](../ocr/04-atallah-lab-spring-2026-log.md)).
...
took ~45 hours ([`docs/ocr/03`](../ocr/03-file-explorer-screenshots.md)), against the
8–26 hour range quoted for runs generally.
